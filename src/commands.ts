...
export function deleteSelection(state: EditorState, dispatch?: (tr: Transaction) => boolean) {
...
export function joinBackward(state: EditorState, dispatch?: (tr: Transaction) => boolean, view?: EditorView) {
...
function textblockAt(node: ProsemirrorNode, side: "start" | "end") {
  for (; node; node = side == "start" ? node.firstChild : node.lastChild) if (node.isTextblock) return true;
  return false;
}
...
export function selectNodeBackward(state: EditorState, dispatch?: (tr: Transaction) => boolean, view?: EditorView) {
...
export function joinForward(state: EditorState, dispatch?: (tr: Transaction) => boolean, view?: EditorView) {
  let { $cursor } = state.selection as any;
...
export function selectNodeForward(state: EditorState, dispatch?: (tr: Transaction) => boolean, view?: EditorView) {
  let { $head, empty } = state.selection,
    $cut = $head;
  if (!empty) return false;
  if ($head.parent.isTextblock) {
...
export function joinUp(state: EditorState, dispatch?: (tr: Transaction) => boolean) {
  let sel = state.selection,
    nodeSel = sel instanceof NodeSelection,
    point: any;
  if (nodeSel) {
    if ((sel as any).node.isTextblock || !canJoin(state.doc, sel.from)) return false;
    point = sel.from;
  } else {
...
export function joinDown(state: EditorState, dispatch?: (tr: Transaction) => boolean) {
  let sel = state.selection,
    point: any;
...
export function lift(state: EditorState, dispatch?: (tr: Transaction) => boolean) {
...
export function newlineInCode(state: EditorState, dispatch?: (tr: Transaction) => boolean) {
...
export function exitCode(state: EditorState, dispatch?: (tr: Transaction) => boolean) {
  let { $head, $anchor } = state.selection;
  if (!$head.parent.type.spec.code || !$head.sameParent($anchor)) return false;
...
export function createParagraphNear(state: EditorState, dispatch?: (tr: Transaction) => boolean) {
  let { $from, $to } = state.selection;
...
// block.
export function liftEmptyBlock(state: EditorState, dispatch?: (tr: Transaction) => boolean) {
  let { $cursor } = state.selection as any;
...
export function splitBlock(state: EditorState, dispatch?: (tr: Transaction) => boolean) {
  let { $from, $to } = state.selection;
...
export function splitBlockKeepMarks(state: EditorState, dispatch?: (tr: Transaction) => boolean) {
  return splitBlock(
    state,
    dispatch &&
      ((tr) => {
...
export function selectParentNode(state: EditorState, dispatch?: (tr: Transaction) => boolean) {
  let { $from, to } = state.selection,
    pos;
...
export function selectAll(state: EditorState, dispatch?: (tr: Transaction) => boolean) {
  if (dispatch) dispatch(state.tr.setSelection(new AllSelection(state.doc)));
  return true;
}

function joinMaybeClear(state: EditorState, $pos: ResolvedPos, dispatch?: (tr: Transaction) => boolean) {
  let before = $pos.nodeBefore,
    after = $pos.nodeAfter,
    index = $pos.index();
  if (!before || !after || (!before.type as any).compatibleContent(after.type)) return false;
...
function deleteBarrier(state: EditorState, $cut: ResolvedPos, dispatch?: (tr: Transaction) => boolean) {
  let before = $cut.nodeBefore,
    after = $cut.nodeAfter,
    conn: any[],
    match: ContentMatch;
...
// attributes.
export function wrapIn(nodeType: NodeType, attrs: { [key: string]: any }) {
...
export function setBlockType(nodeType: NodeType, attrs: { [key: string]: any }) {
  return function (state: EditorState, dispatch?: (tr: Transaction) => boolean) {
    let { from, to } = state.selection;
...
function markApplies(doc: ProsemirrorNode, ranges: any, type: MarkType) {
...
// document.
export function toggleMark(markType: MarkType, attrs: { [key: string]: any }) {
  return function (state: EditorState, dispatch?: (tr: Transaction) => boolean) {
    let { empty, $cursor, ranges } = state.selection as any;
...
export function chainCommands(
  ...commands: Array<(state: EditorState, dispatch?: (tr: Transaction) => boolean, view?: EditorView) => boolean>
) {
  return function (state: EditorState, dispatch?: (tr: Transaction) => boolean, view?: EditorView) {
